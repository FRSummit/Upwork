  ...
  <div>
    <v-card class="elevation-12 v-card-edit">
      <v-toolbar color="primary" dark flat>
        <v-spacer />
        <v-toolbar-title>Add New Date here</v-toolbar-title>
        <v-spacer />
      </v-toolbar>
      <v-card-text class="v-card-text">
        <v-form @submit.prevent="onSubmit">
          <div class="name-field">
            <label class="field-label" for="name" style="display: inline-block; width: 30%; text-align: center;">Date</label>
            <v-text-field class="field-input" v-model="date" id="date" placeholder="Date" type="text" style="display: inline-block; width: 60%;"/>
          </div>
          <div class="name-field">
        ...
        <table style="">
            ...
            <tbody style="background-color: #a09c9c;">
                <tr v-for="(impDate, key) in impDates" :key="key">
                    <td style="width: 35%; padding: 2px;">{{ impDate.date }}</td>
...
export default {
  ...
  methods: {
    onSubmit() {
      ...
      if(this.date === "" || this.date === null || this.description === "" || this.description === null) {
          console.log('null')
        if(this.date == "" || this.date === null) {
          document.getElementById('date').placeholder = 'Date must not be empty';
        } if(this.description == "" || this.description === null) {
          document.getElementById('description').placeholder = 'Description must not be empty';
        }
      }else {
          console.log('firebase')
        firebase.database().ref('homeSidebarImportantDates').push({
          date: this.date,
          description: this.description
        })
        .then((data)=>{
    ...
    deleteImpDate(id) {
      console.log(id)
      firebase.database().ref('homeSidebarImportantDates/' + id).remove();
    }
  },
}
</script>
